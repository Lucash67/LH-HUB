...
export function HeroMetric({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative overflow-hidden rounded-2xl border bg-surface-card p-5 shadow-card transition-all duration-200 hover:shadow-glow",
        t.border,
      )}
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className={cn("text-3xl font-bold tracking-tight sm:text-4xl", valueColor)}>{formatted}</p>
          {trend !== undefined && (
            <p
              className={cn(
        ...
        <div className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-xl", t.accentDim)}>
          <Icon className={cn("h-5 w-5", t.accent)} strokeWidth={1.75} />
        </div>
      </div>
    </motion.div>
  );
