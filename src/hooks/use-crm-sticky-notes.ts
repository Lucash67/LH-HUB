...
import type { SaveStatus, StickyNotePatch } from "@/hooks/use-sticky-notes";
import {
  localClearPending,
  localDeleteNote,
  localGetPendingIds,
  ...
  localPutNotes,
  mergeNotes,
} from "@/lib/crm/notes-local-store";

export type { SaveStatus, StickyNotePatch };

const SAVE_DEBOUNCE_MS = 450;

async function fetchNotes(includeArchived = false): Promise<StickyNote[]> {
  const qs = includeArchived ? "?archived=1" : "";
  const res = await fetch(`/api/crm/notes${qs}`, { credentials: "include" });
...
export function useCrmStickyNotes() {
  ...
  const deleteNote = useCallback(async (id: string) => {
    ...
    pendingRef.current.delete(id);
    try {
      await fetch(`/api/crm/notes?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
        credentials: "include",
        keepalive: true,
      });
