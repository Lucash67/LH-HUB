/**
 * Sprint A.3.1 — Auditoria READ-ONLY do dia 16/07/2026
 */
...
const report = {
  sales: db.prepare(
    `SELECT s.time, c.name, s.total_amount, s.payment_status, s.amount_received, s.payment_method
     FROM sales s LEFT JOIN clients c ON c.id = s.client_id
     WHERE s.date = ? AND s.business_id = ? ORDER BY s.time`,
  ).all(DATE, BUSINESS),
  products: db.prepare(
  ...
  investment: db.prepare(`SELECT * FROM investments WHERE date = ? AND business_id = ?`).get(DATE, BUSINESS),
  diaryOp: db.prepare(
  ...
  purchase: db.prepare(
    `SELECT * FROM daily_purchases WHERE business_id = ? AND date = ?`,
  ).get(BUSINESS, DATE),
  losses: db.prepare(`SELECT * FROM operational_losses WHERE date = ?`).all(DATE),
